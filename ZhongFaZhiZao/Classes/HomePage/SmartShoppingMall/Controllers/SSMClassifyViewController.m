...
//    self.imageNameStr = @"MineAuxiliaryIcon";//默认功能管理辅助视图的图片的名字
    CGFloat titleViewHeight = (80 + 20 * 2)/2.0 * KWidth_ScaleH;
    ...
    UILabel *titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(20 * KWidth_ScaleW, topGapView.bottom + 13/2.0 * KWidth_ScaleH, 100, 45/2.0 * KWidth_ScaleH)];
    ...
    openOrCloseImageView.image = [UIImage imageNamed:@"MineAuxiliaryIcon"];
    [titleView addSubview:openOrCloseImageView];
    
    UIView *gapLineView = [[UIView alloc] initWithFrame:CGRectMake(0, titleViewHeight - 20/2.0 * KWidth_ScaleH, kScreenWidth, 20/2.0 * KWidth_ScaleH)];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    SSMClassifyDatas *classifyDatas = self.theDatas[indexPath.section];
    
    NSInteger theSection = classifyDatas.subclass.count / 2;
    CGFloat height = 0.;
    /*
    if ([_flagArray[indexPath.section] isEqualToString:@"0"]) {
        return height;
    }else{
        height = (80 * theSection + 10 * theSection)/2.0 * KWidth_ScaleH;
    }*/
    height = (80 * theSection + 10 * theSection)/2.0 * KWidth_ScaleH;
    return height;
}
...
//    [titleView addSubview:topGapView];
    ...
    UILabel *titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(20, 13/2.0 * KWidth_ScaleH, 100, 45/2.0 * KWidth_ScaleH)];
    titleLabel.textColor = [UIColor colorWithHexString:@"#1C1C1C"];
    titleLabel.font = [UIFont systemFontOfSize:KFloat(16)];
    [titleView addSubview:titleLabel];
...
//    self.openOrCloseFounctionManagerImg = openOrCloseImageView;
    ...
    UIView *gapLineView = [[UIView alloc] initWithFrame:CGRectMake(0, titleViewHeight - 0.5, kScreenWidth, 0.5)];
    gapLineView.backgroundColor = UIColorFromRGBA(245, 245, 245, 1.0);
    [titleView addSubview:gapLineView];
        
    titleView.tag = 101 + section;
    titleView.userInteractionEnabled = YES;
...
- (void)sectionClick:(UITapGestureRecognizer *)tap{
    ...
    tableVC.level = @"1";
    if (tap.view.tag != 100) {
        SSMClassifyDatas *classifyDatas = self.theDatas[tap.view.tag - 101];
        tableVC.classid = classifyDatas.classifyId;
    }else{
        tableVC.level = @"";
    }
    for (id subView in tap.view.subviews) {
        if ([subView isKindOfClass:[UILabel class]]) {
            UILabel *currentLabel = (UILabel *)subView;
