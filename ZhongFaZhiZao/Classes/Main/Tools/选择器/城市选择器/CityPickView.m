...
+ (CityPickView *)showPickViewWithDefaultProvince:(NSString *)province withCity:(NSString *)cityStr withTown:(NSString *)townStr withThreeScroll:(BOOL)threeScroll complete:(CityPickViewBlock)block{
    ...
    CityPickView *pickView= [[CityPickView alloc] initWithFrame:CGRectMake(0, 0, screenWitdth, screenHeight)];
...
- (void)loadData{
    ...
    for (NSDictionary *dic in arrData) {
        ///此处用到底 "YYModel"
        TheProvince *provice = [TheProvince yy_modelWithDictionary:dic];
...
- (void)setupUI{
    ...
    cancelButton.font = [UIFont systemFontOfSize:KFloat(14)];
    ...
    sureButton.titleLabel.font = [UIFont systemFontOfSize:KFloat(18)];
...
- (NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component{
    ...
    TheProvince *province = self.allProvinces[self.rowOfProvince];
    TheCity *city = province.city[self.rowOfCity];
...
- (nullable NSString *)pickerView:(UIPickerView *)pickerView titleForRow:(NSInteger)row forComponent:(NSInteger)component{
    ...
    if (component==0){//省
        TheProvince *province = self.allProvinces[row];
        showTitleValue = province.name;
    }
    if (component==1){//市
        TheProvince *province = self.allProvinces[self.rowOfProvince];
        TheCity *city = province.city[row];
        ...
        if (self.rowOfCity < province.city.count) {
            ...
            if (scroll == YES){
                if (self.rowOfTown < city.town.count) {
                    TheTown *town = city.town[self.rowOfTown];
...
-(void)scrollToRow:(NSInteger)firstRow  secondRow:(NSInteger)secondRow thirdRow:(NSInteger)thirdRow{
    if (firstRow < self.allProvinces.count) {
        ...
        TheProvince *province = self.allProvinces[firstRow];
        if (secondRow < province.city.count) {
            self.rowOfCity = secondRow;
            [self.pickerView reloadComponent:1];
            
            if (scroll == YES){
                TheCity *city = province.city[secondRow];
...
- (NSInteger)rowOfProvinceWithName:(NSString *)provinceName{
    ...
    NSInteger row = 0;
    for (TheProvince *province in self.allProvinces) {
...
- (NSInteger)rowOfCityWithName:(NSString *)cityName{
    ...
    TheProvince *province = self.allProvinces[recordRowOfProvince];
    
    for (TheCity *city in province.city) {
