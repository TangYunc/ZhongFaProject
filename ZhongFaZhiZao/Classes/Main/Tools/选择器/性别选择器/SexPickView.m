...
#import "SexPickView.h"

#define currentMargin  6
#define ButtonHeight  50
#define TitleHeight   50
#define LineHeight    0.5

@interface SexPickView ()

@property (nonatomic, assign) CGFloat toolbarH;

@end

@implementation SexPickView

- (id)initWithTitle:(NSString *)title buttons:(NSArray<NSString *> *)buttons buttonClick:(void (^)(SexPickView *, NSInteger))block{
    ...
    if (self = [super init]) {
        ...
        _toolbarH = buttons.count*(ButtonHeight+LineHeight)+(buttons.count>1?currentMargin:0)+(title.length?TitleHeight:0);
        ...
        [buttons enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            if (idx==buttons.count-1&&buttons.count>1) {
                button.frame = (CGRect){0,buttonMinY+(ButtonHeight+LineHeight)*idx+currentMargin,CGRectGetWidth(self.frame),ButtonHeight};
            }else{
